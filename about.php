<?php $title = 'About'; ?>
<?php include 'header.php';?>
<?php include 'articleheader.php'; ?>

<ul class="breadcrumb">
    <li>Home</li>
    <li>Our Group</li>
    <li class="active">About</li>
</ul>

<h2 class="page-header">About</h2>

    <p>
        The Cooleman Ridge Park Care Group is a <a href="http://www.environment.act.gov.au/parks-conservation/parks-and-reserves/get-involved/parkcare-and-urban-landcare-program">Park Care</a> group that &quot;cares&quot; for the Cooleman Ridge Nature Reserve.
    </p>

    <p>
        The Cooleman Ridge Park Care Group is part of a larger network of other parkcare groups working throughout the ACT. See our <a href="partners.php">partners</a> page for more details.
    </p>

    <p>
        You can read more about the important work that group performs in the Our Work section of the web site. Blah
    </p>

    <h3>Mission</h3>

    <p>
        The mission of the Cooleman Ridge Park Care Group is to &quot;regenerate the bush&quot; by returning the ecology of the nature reserve to its pre-European state. Please see our <a href="strategy.php">Strategy</a> page for more details about how the group is working to achieve this goal.
    </p>

    <h3>History</h3>

    <p>
        The Cooleman Ridge Park Care Group is small but active group made up of various volunteers from a variety of backgrounds, such as scientists, public servants, homekeepers. The group was formed in 1991 by three ladies - Bess Sledge, Pauline Lynga and Tina Nevins -  and now grown to have 50 registered members that contribute in various ways to the groups objectives.
    </p>

    <h3>Achievements</h3>

    <p>The group has made significant progress with rehabilitating the reserve but much work remains to be done.</p>

    <p>the achievements are not always obvious.</p>

    <p>
        The major achievments of the group are:

        <ul>
            <li>Significantly reduced the presence of weeds in the many of the keay areas on the ridge.</li>
            <li>Surveyed many of the significant aspects of the ridge</li>

        </ul>
    </p>

    <p>
        The group has managed to make some major improvements to the ecology of the ridge. Native habit has been progressively replaced damaged errors. There is still a signficant amoun of work to be done.
    </p>

    <p>
        The Cooleman Ridge Nature Reserve is one of the more damaged but there still remain many good areas. These areas serve as the nucleus of the groups efforts to restore the ecology of the ridge.
    </p>

<?php include 'articlefooter.php'; ?>
<?php include 'footer.php'; ?>

