<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<!-- saved from url=(0038)news.html -->
<html>

<head>
<title>News</title>
<meta content="text/html; charset=windows-1252" http-equiv="Content-Type">
<meta content="MSHTML 9.00.8112.16443" name="GENERATOR">
<link rel="stylesheet" href="styles/style.css" type="text/css">
</head>

<body leftmargin="0" topmargin="0">

<?php include 'header.php'; ?>
<table cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td width="20">&nbsp;</td>
		<td><br>
		<h2><font color="#ba7025" face="ARIAL">Newsletter</font></h2>
		</td>
		<td width="20">&nbsp;</td>
	</tr>
	<tr>
		<td width="20">&nbsp;</td>
		<td valign="top">
		<p>Our <a href="news/news.pdf">newsletter</a> is available in Adobe pdf 
		format. It contains news of our activities, recent vegetation growth and 
		fauna sitings, as well as interesting features on plant identification, 
		water quality and other info.</p>
		<p>A regular feature is the "What's Around" segment, which includes a couple 
		of pictures of plants with their names and a short description.</p>
		<p>Some <a href="previous.php">previous newsletters</a> are available in 
		pdf format.</p>
		<p>Some <a href="other.php">other important documents</a> such as correspondence, 
		reports, etc are also available.</p>
		<p>To return to this web page from the newsletter, you'll probably need 
		to click on the BACK button on your web browser's toolbar (different browsers 
		may vary, but this is usually at the top left of your screen).</p>
		</td>
		<td width="20">&nbsp;</td>
	</tr>
	<tr>
		<td colspan="3">&nbsp;</td>
	</tr>
</table>
<?php include 'footer.php'; ?>
</body>

</html>
